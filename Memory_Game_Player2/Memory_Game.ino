...
int playerRole = 2;  // Set to 2 for Player 2
uint8_t peerAddress[] = {0xd4, 0xd4, 0xda, 0x79, 0x26, 0xe8};  // Set to Player 1's MAC address
...
void onButtonPress() {
    if (digitalRead(SUBMIT_BUTTON_PIN) == LOW) {
        sendData("Player 2 is ready!");
        player2Ready = true;
        updateOLED("Player 2", "Waiting for Player 1...");
...
void OnDataRecv(const esp_now_recv_info *info, const uint8_t *incomingData, int len) {
    ...
    if (player1Ready && player2Ready && !gameStarted) {
     ...
     // If Player 2 receives a bomb message
    if (strcmp(myData.message, "Bomb Used") == 0) {
        if (playerRole == 2) {
            ...
            // Send the updated health back to Player 1
            sendData("Health Updated");

            // Update Player 2's display and show bomb animation
            updateScoresAndHealthDisplay();
            drawBombAnimation();
        }
    }

    // If Player 1 receives Player 2's updated health
    if (strcmp(myData.message, "Health Updated") == 0 && playerRole == 1) {
        // Player 1 updates Player 2's health on their screen
        player2Health = myData.player2Health;
        updateScoresAndHealthDisplay();
    }

    if (strcmp(myData.message, "Game Over Player1") == 0) {
        // If Player 1 loses, Player 2 will show victory
        if (playerRole == 2) {
            display.clearDisplay(); 
            updateOLED("You win!", "Player 1 Defeated");
            gameOver = true;
            soundBuzzer();
            resetGame();
        }
    ...
    if (gameStarted && !gameOver) {
        ...
        if (digitalRead(SUBMIT_BUTTON_PIN) == LOW) {
            ...
            if (inputIndex != sequenceLength) {
                ...
                lightUpRing(255, 0, 0);
                player2Health -= 5;
                updateOLED("Health", String(player2Health));
                bombSequence = 0;
                bombReady = false;
                patternSpeed = 1000;
                pointsPerRound = 1;
...
void checkPlayerInput() {
 ...
 if (correct) {
   ...
   updateOLED("Score", String(player2Score));
   lightUpRing(0, 255, 0);
   bombSequence++;
   if (bombSequence >= 2) {
     bombReady = true;  // score of 3 in a row makes the bomb ready
...
void useBomb() {
    ...
    // Send the bomb activation message to Player 2
    bombActivated = true;
    sendData("Bomb Used");

    bombReady = false;  // Bomb is no longer ready after use

    // Update Player 1's display with the latest info
    updateScoresAndHealthDisplay();
